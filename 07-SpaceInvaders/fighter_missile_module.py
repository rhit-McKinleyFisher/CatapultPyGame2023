...
class Missile:
    ...
    def draw(self):
        ...
        pygame.draw.line(self.screen, (0, 225, 0), (self.x, self.y), (self.x, self.y + 8), 4)
...
class Fighter:
    ...
    def move(self, move_amount_x):
        ...
        if self.x > self.screen.get_width() + self.image.get_width() - 100:
            self.x = 0 - self.image.get_width()
        if self.x < 0 - self.image.get_width():
            self.x = self.screen.get_width() + self.image.get_width() - 100
